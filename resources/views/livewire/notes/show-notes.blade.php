...
<div>
    <div class="space-y-2">
        ...
        <div class="text-center">
            ...
            <x-button primary right-icon="calendar" class="mt-6" href="{{ route('notes.create') }}" wire:navigate>Criar anotação</x-button>
        </div>


        @else
        <x-button primary right-icon="calendar" class="mt-6 mb-12" href="{{ route('notes.create') }}" wire:navigate>Criar anotação</x-button>
            ...
            @foreach ($notes as $note)
                <x-card wire:key='{{ $note->id }}'>
                    ...
                    <div class="flex items-end justify-between mt-4 space-x-1">
                        <p class="text-xs">Remetente: <span class="font-semibold">{{ $note->recipient }}</span></p>
                        <div>
